...
def get_BSpline_decomposition(target_function, multivariate_samples, BSpline_list=None, d=None, order=4, Constraint="Concavity", knots=None, a=None, b=None, lam=1e-2):
    ...
    if a is None or b is None:
        ...
        b = sorted_samples[-1]
    if knots is None:
        knots = np.concatenate([[a[d]]*order, sorted_samples, [b[d]]*order])
    ...
    Y = np.array([target_function(multivariate_samples[i])-sum(B(multivariate_samples[i,k]) for k,B in enumerate(BSpline_list) if k !=d ) for i in range(N)])
